FactoryBot.define do
  factory :early_career_payments_eligibility, class: "EarlyCareerPayments::Eligibility" do
    ...
    trait :ineligible_now_but_eligible_next_year do
      ...
      itt_academic_year { JourneyConfiguration.for(Policies::EarlyCareerPayments).current_academic_year - 4 } # this makes it ineligible
    end

    trait :eligible_now_and_again_but_two_years_later do
      eligible_now_with_mathematics
      itt_academic_year { JourneyConfiguration.for(Policies::EarlyCareerPayments).current_academic_year - 3 }
    end
    ...
    trait :eligible_itt_subject_later do
      itt_academic_year { JourneyConfiguration.for(Policies::EarlyCareerPayments).current_academic_year - 4 }
      eligible_itt_subject { :mathematics }
    end
    ...
    trait :no_eligible_subjects do
      eligible_now
      itt_academic_year { JourneyConfiguration.for(Policies::EarlyCareerPayments).current_academic_year - 1 }
    end
