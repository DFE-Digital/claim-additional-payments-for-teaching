  ...
  describe ".process" do
    ...
    context "when there are existing records" do
      ...
      it "populates the table with the CSV data" do
        expect { importer.process }.to change { Policies::TargetedRetentionIncentivePayments::Award.count }.from(2).to(10)
        expect(Policies::TargetedRetentionIncentivePayments::Award.by_academic_year(academic_year).count).to eq 8
      end
      ...
      context "when there are errors" do
        ...
        it "does not populate the table with the CSV data" do
          importer.process
          expect(Policies::TargetedRetentionIncentivePayments::Award.by_academic_year(academic_year).count).to eq 1
        end
      end
    end
  end
