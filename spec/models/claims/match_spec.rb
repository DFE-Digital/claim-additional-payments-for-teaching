...
RSpec.describe Claims::Match do
  ...
  describe "#other" do
    it "raises when the claim is not part of the match" do
      claim_1 = create(
        :claim,
        email_address: "e.krabapple@springfield-elementary.edu",
        created_at: 2.days.ago
      ...
      unrelated_claim = create(:claim)

      match = described_class.create_match!(claim_1, claim_2)

      expect { match.other(unrelated_claim) }.to raise_error(
        ArgumentError,
        "claim is not part of this match"
      )
    end
  end
end
