  ...
  context "The CSV has a claim missing from the run" do
    ...
    it "fails and populates its errors, and does not update the payments" do
      ...
      expect(payment_confirmation.errors).to eq([
        "Payment ID #{payroll_run.payments[1].id} is missing from the file. Please check with DfE Payroll to see if there is a problem with this payment. You may need to remove some payments from the Payroll Run then try uploading it again"
      ])
