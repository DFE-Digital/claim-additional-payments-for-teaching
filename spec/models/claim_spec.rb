...
RSpec.describe Claim, type: :model do
  ...
  context "that has bank details" do
    ...
    describe "HMRC API account validation", :with_stubbed_hmrc_client do
      let(:claim) { build(:claim, :with_bank_details, policy: EarlyCareerPayments) }

      [:"personal-bank-account", :"building-society-account"].each do |context|
        context "validating in #{context} context" do
          context "when HMRC bank validation is enabled", :with_hmrc_bank_validation_enabled do
            before { claim.valid?(context) }
            ...
            context "when there is an error with the sort code" do
              ...
              it "adds an error" do
                expect(claim.errors[:bank_sort_code].first).to eq("Enter a valid sort code")
              end
            end
            ...
            context "when there is an error with the account name" do
              ...
              it "adds an error" do
                expect(claim.errors[:banking_name].first).to eq("Enter a valid name on the account")
              end
            end
            ...
            context "when there is an error with the account number" do
              ...
              it "adds an error" do
                expect(claim.errors[:bank_account_number].first).to eq("Enter the account number associated with the name on the account and/or sort code")
              end
            end

            context "when there is an HMRC API error", :with_failing_hmrc_bank_validation do
              it "does not add any errors" do
                expect(claim.errors[:bank_sort_code]).to be_empty
                expect(claim.errors[:bank_account_number]).to be_empty
                expect(claim.errors[:banking_name]).to be_empty
              end

              it "catches the exception" do
                expect { claim.valid?(context) }.not_to raise_error
              end
            end
          end
