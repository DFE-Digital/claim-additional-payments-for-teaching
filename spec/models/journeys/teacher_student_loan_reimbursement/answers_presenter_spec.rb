...
RSpec.describe Journeys::TeacherStudentLoanReimbursement::AnswersPresenter, type: :model do
  ...
  describe "#eligibility_answers" do
    ...
    context "qualifications retrieved from DQT" do
      ...
      it "removes the QTS question" do
        expect(answers).not_to include([I18n.t("student_loans.questions.qts_award_year"), "Between the start of the 2013 to 2014 academic year and the end of the 2020 to 2021 academic year", "qts-year"])
      end
    end
  end
  ...
  describe "#student_loan_answers" do
    ...
    let(:claim) { build(:claim, policy:, eligibility:) }

    subject(:answers) { described_class.new(current_claim).student_loan_answers }

    it "returns an array of question and answers including the student loan amount" do
      expect(answers).to include([I18n.t("student_loans.questions.student_loan_amount", financial_year: Policies::StudentLoans.current_financial_year), "£1,987.65", "student-loan-amount"])
    end
  end
end
