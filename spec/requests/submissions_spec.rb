...
RSpec.describe "Submissions", type: :request do
  ...
  describe "#create" do
    context "with a submittable claim" do
      ...
      context "without ENV['DQT_API_HOST']" do
        def without_env_dqt_api_host
          current_env_dqt_api_host = ENV["DQT_API_HOST"]
          ENV["DQT_API_HOST"] = nil
          yield
          ENV["DQT_API_HOST"] = current_env_dqt_api_host
        end

        it "does not enqueue UpdateAdminClaimTasksWithDqtApiJob" do
          without_env_dqt_api_host do
            expect { post claim_submission_path(StudentLoans.routing_name) }.not_to(
              have_enqueued_job(UpdateAdminClaimTasksWithDqtApiJob)
            )
          end
        end
