...
RSpec.feature "Levelling up premium payments and early-career payments combined claim journey", :with_stubbed_hmrc_client, :with_hmrc_bank_validation_enabled do
  ...
  scenario "Eligible for both" do
    ...
    visit claim_completion_path(Policies::EarlyCareerPayments.routing_name)
    expect(page).to have_current_path("/#{Policies::EarlyCareerPayments.routing_name}/landing-page")

    click_on "Start now"
    expect(page).to have_current_path("/#{Policies::EarlyCareerPayments.routing_name}/existing-session")
    ...
    # Check we can't skip ahead pages in the journey
    visit claim_path(Policies::EarlyCareerPayments.routing_name, "nqt-in-academic-year-after-itt")
    expect(page).to have_current_path("/#{Policies::EarlyCareerPayments.routing_name}/current-school")
    ...
    visit claim_path(Policies::EarlyCareerPayments.routing_name, "email-address")
    expect(page).to have_current_path("/#{Policies::EarlyCareerPayments.routing_name}/address")
