...
RSpec.feature "Admin claim support tickets" do
  ...
  scenario "the service operator adds a support ticket to a claim" do
    ...
    expect(page).to have_link("View all support tickets in Zendesk by applicant email", href: "https://becomingateacher.zendesk.com/agent/search/1?copy&type=ticket&q=person1%40example.com")
    expect(page).to have_link("View all support tickets in Zendesk by claim reference", href: "https://becomingateacher.zendesk.com/agent/search/1?copy&type=ticket&q=custom_field_19564949397009:#{claim.reference}")
