...
RSpec.feature "Provider verifying claims", feature_flag: :provider_dashboard do
  scenario "with no eligible courses" do
    fe_provider = create(
      :school,
      :further_education,
    ...
    expect(page).to have_text("Which subject areas does Edna Krabappel teach?")
    ...
    check "They do not teach any of these courses"
    click_on "Continue"
    ...
    expect(summary_row("Subject areas")).to have_content("Building and construction")

    expect(page).to have_summary_item(
      key: "Building and construction",
      value: "They do not teach any of these courses"
    )

    expect(page).not_to have_text "Spend at least half timetabled teaching time teaching relevant courses"
