...
RSpec.feature "Further education payments" do
  ...
  scenario "submitting multiple times at end of journey" do
    when_student_loan_data_exists
    when_further_education_payments_journey_configuration_exists
    and_college_exists
    ...
    existing_claim = create(
      ...
      :submitted,
      :further_education,
      onelogin_uid: Journeys::FurtherEducationPayments::Session.last.answers.onelogin_uid,
      academic_year: Journeys::FurtherEducationPayments::Session.last.answers.academic_year
    )
    ...
    expect(page).to have_content("You’ve already submitted a claim in this claim window")
    expect(page).to have_content("The claim you submitted has the reference number #{existing_claim.reference}")
  end
