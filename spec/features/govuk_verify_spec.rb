...
RSpec.feature "Teacher verifies identity using GOV.UK Verify" do
  ...
  context "handling errors", js: true do
    before do
      stub_vsp_translate_response_request(error_type)
      click_on "Continue to GOV.UK Verify"
      click_on "Perform identity check"
    end

    context "cancelled response" do
      let(:error_type) { "no-authentication" }

      scenario "shows the correct error" do
        expect(page).to have_text(I18n.t("verify.errors.exited.title"))
        expect(page).to have_text(I18n.t("verify.errors.exited.description"))
      end
    end

    context "failed response" do
      let(:error_type) { "authentication-failed" }

      scenario "shows the failed error" do
        expect(page).to have_text(I18n.t("verify.errors.failed.title"))
        expect(page).to have_text(I18n.t("verify.errors.failed.description"))
      end
    end
    ...
    context "errored response" do
      ...
      scenario "shows the failed error" do
        expect(page).to have_text(I18n.t("verify.errors.error.title"))
        expect(page).to have_text(I18n.t("verify.errors.error.description"))
      end
    end
  end
end
