...
RSpec.feature "TSLR journey with Teacher ID school playback" do
  ...
  scenario "Shows and selects suggested eligible claim school" do
    ...
    fill_in "Which school are you currently employed to teach at?",
      with: other_eligible_school.name
    click_on "Continue"

    expect(page).to have_text "Which school are you currently employed to teach at?"
    choose other_eligible_school.name
    click_on "Continue"

    expect(page).to have_text "Were you employed in a leadership position"
  end
