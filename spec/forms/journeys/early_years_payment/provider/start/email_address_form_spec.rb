...
RSpec.describe Journeys::EarlyYearsPayment::Provider::Start::EmailAddressForm do
  subject(:form) { described_class.new(journey:, journey_session:, params:) }

  let(:journey) { Journeys::EarlyYearsPayment::Provider::Start }
  let(:journey_session) { build(:early_years_payment_provider_start_session) }
