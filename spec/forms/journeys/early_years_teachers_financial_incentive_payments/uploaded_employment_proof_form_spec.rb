...
RSpec.describe Journeys::EarlyYearsTeachersFinancialIncentivePayments::UploadedEmploymentProofForm do
  let(:journey) { Journeys::EarlyYearsTeachersFinancialIncentivePayments }
  let(:blob) { create(:active_storage_blob) }

  let(:journey_session) do
    create(:eytfi_session, answers: {confirmed_employment_proof_blob_ids: [blob.id.to_s]})
  end
