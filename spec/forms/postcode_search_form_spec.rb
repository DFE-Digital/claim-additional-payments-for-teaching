...
RSpec.describe PostcodeSearchForm, type: :model do
  ...
  describe "#save" do
    context "when the postcode lookup failed" do
      let(:form) { subject }
      let(:params) do
        ActionController::Parameters.new(claim: {postcode: "SW1B 1AA"})
      end
      ...
      before do
        ...
        stub_request(
          :get,
          "https://api.os.uk/search/places/v1/postcode?key=ABC123&postcode=SW1B1AA"
        )
          .to_return(
            status: 500,
            body: ""
      ...
      it "stores state to session" do
        ...
        expect(journey_session.reload.answers.ordnance_survey_error).to eq true
        expect(journey_session.reload.answers.skip_postcode_search).to eq nil
      end
    end
  end
end
