...
RSpec.describe FurtherEducationPayments::Providers::Claims::Verification::SubjectsTaughtForm, type: :model do
  let(:fe_provider) do
  ...
  describe "validations" do
    context "when submission" do
      it do
        is_expected.not_to(
          allow_value(nil).for(:provider_verification_subjects_taught)
        )
      end

      it do
        is_expected.to(
          allow_value(true).for(:provider_verification_subjects_taught)
        )
      end

      it do
        is_expected.to(
          allow_value(false).for(:provider_verification_subjects_taught)
        )
      end
    end
