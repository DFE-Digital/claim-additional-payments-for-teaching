...
# to multiple claims for different policies if a `JourneyConfiguration` specifies multiple policies.
#
...
class CurrentClaim
  ...
  def policy_year
    raise "nil academic year" if policies.any? { |policy| JourneyConfiguration.for(policy).current_academic_year.nil? }
    raise "none academic year" if policies.any? { |policy| JourneyConfiguration.for(policy).current_academic_year == AcademicYear.new }

    policy_year_values_set = policies.collect { |policy| JourneyConfiguration.for(policy).current_academic_year }.to_set
