class PaymentConfirmation
  ...
  def scheduled_payment_date
    ...
    (end_of_month.wday == 5) ? end_of_month : end_of_month.prev_occurring(:friday)
  end
  ...
  def check_for_missing_payments
    ...
    missing_payment_ids.each do |id|
      errors.append("Payment ID #{id} is missing from the file. Please check with DfE Payroll to see if there is a problem with this payment. You may need to remove some payments from the Payroll Run then try uploading it again")
    end
  end
