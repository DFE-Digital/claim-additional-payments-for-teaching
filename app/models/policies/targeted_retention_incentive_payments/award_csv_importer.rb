    ...
    class AwardCsvImporter
      ...
      def commit!
        TargetedRetentionIncentivePayments::Award.transaction do
          TargetedRetentionIncentivePayments::Award.where(academic_year: academic_year.to_s).destroy_all
          @records.each(&:save!)
        end
      end
      ...
      def csv_rows_are_valid
        parse_csv.each.with_index(1) do |row, line|
          ...
          record = TargetedRetentionIncentivePayments::Award.new(row.to_h.merge(academic_year: academic_year))
          @records << record
