    ...
    class AwardCsvImporter
      ...
      def file_upload
        @file_upload ||= FileUpload.new(
          ...
          target_data_model: Policies::TargetedRetentionIncentivePayments::Award.to_s,
          academic_year: academic_year.to_s
        )
      end

      def commit!
        TargetedRetentionIncentivePayments::Award.transaction do
          @records.each(&:save!)
        end
      end
      ...
      def csv_rows_are_valid
        ...
        parse_csv.each.with_index(1) do |row, line|
          ...
          record = TargetedRetentionIncentivePayments::Award.new(row.to_h.merge(academic_year: academic_year, file_upload_id: file_upload.id))
          @records << record
