# Run me with `rails runner db/data/20260730112619_persist_task_identity.rb`
...
relevant_policies.each do |policy|
  ...
  tasks = Task.where(name: "identity_confirmation", claim: claims)
  ...
  claims_without_task.find_each do |claim|
    ...
    verifier = AutomatedChecks::ClaimVerifiers::Identity.new(
      claim:,
      dqt_teacher_status:
    )
    verifier.perform
