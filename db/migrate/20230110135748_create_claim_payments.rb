class CreateClaimPayments < ActiveRecord::Migration[6.1]
  def up
    create_table :claim_payments, id: :uuid do |t|
      t.references :claim, type: :uuid, foreign_key: true
      t.references :payment, type: :uuid, foreign_key: true

      t.timestamps
    end
    add_index :claim_payments, [:claim_id, :payment_id], unique: true

    execute("INSERT INTO claim_payments (claim_id, payment_id, created_at, updated_at) SELECT id AS claim_id, payment_id, created_at, created_at FROM claims WHERE payment_id IS NOT NULL")
